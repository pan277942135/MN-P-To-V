...
function inputError(code: 'INPUT_IMAGE_INVALID' | 'PROMPT_INVALID' | 'IDENTITY_INPUT_UNSAFE', message: string, technicalMessage?: string): MvpStructuredError {
  return {
    code,
    stage: 'input',
    message,
...
async function submitVeoOnce(params: {
  ...
  const body = {
    instances: [{
      ...
      image: { bytesBase64Encoded: params.imageBuffer.toString('base64'), mimeType: params.mimeType },
    }],
    parameters: {
      sampleCount: 1,
      aspectRatio: '9:16',
...
async function persistInputReference(taskId: string, buffer: Buffer, mimeType: string): Promise<string> {
  const objectPath = inputObjectPath(taskId, mimeType);
  await storage.bucket(OUTPUT_BUCKET).file(objectPath).save(buffer, {
    resumable: false,
    metadata: { contentType: mimeType, metadata: { purpose: 'identity-reference' } },
  });
  const verify = await exactDownload(OUTPUT_BUCKET, objectPath);
  if (!verify.equals(buffer)) throw new Error('Persisted identity reference failed exact-read verification.');
  return objectPath;
}
...
async function persistAttemptAuditArtifact(taskId: string, attempt: number, videoBuffer: Buffer): Promise<void> {
  ...
  await storage.bucket(OUTPUT_BUCKET).file(auditAttemptPath(taskId, attempt)).save(videoBuffer, {
    resumable: false,
    metadata: { contentType: 'video/mp4', metadata: { purpose: 'identity-qa-failed-attempt', attempt: String(attempt) } },
  });
}
...
async function createTaskIdempotently(params: {
  ...
  return await db.runTransaction(async (tx) => {
    ...
    const task: MvpVideoTask = {
      ...
      identitySafeMode: params.identitySafeMode,
      durationSeconds: params.durationSeconds,
      ...
      identityQaModel: params.identitySafeMode ? IDENTITY_QA_MODEL : null,
      identityReport: null,
...
function classifySubmitFailure(error: unknown): { status: 'FAILED' | 'SUBMISSION_OUTCOME_UNKNOWN'; detail: MvpStructuredError } {
  ...
  return { status: 'SUBMISSION_OUTCOME_UNKNOWN', detail: { code: 'SUBMISSION_OUTCOME_UNKNOWN', stage: 'submit', message, retryable: false, recommendedAction: '不要重新提交同一任务；先使用【恢复产物】检查当前 attempt 的任务专属 GCS 前缀。', technicalMessage: message } };
}

async function completeFromBuffer(task: MvpVideoTask, buffer: Buffer, identityReport?: MvpIdentityQaReport | null): Promise<MvpVideoTask> {
  await updateTask(task.taskId, { status: 'SAVING', stage: 'artifact_persist', error: null, identityReport: identityReport ?? task.identityReport ?? null });
  try {
    ...
    assertIdentitySafeCompletion(candidate);
    ...
    return await updateTask(task.taskId, {
      ...
      artifactPersisted: false,
      artifactVerified: false,
      error: { code: 'OUTPUT_PERSIST_FAILED', stage: 'artifact_persist', message, retryable: true, recommendedAction: '检查 GCS 权限/配额后恢复或重新生成。', technicalMessage: message },
    });
  }
}

async function processGeneratedBuffer(task: MvpVideoTask, buffer: Buffer, allowIdentityRetry: boolean): Promise<MvpVideoTask> {
  if (task.identitySafeMode !== true) return completeFromBuffer(task, buffer, null);
  const attempt = task.providerAttempt || 1;
  task = await updateTask(task.taskId, { status: 'QUALITY_CHECKING', stage: 'identity_qa', error: null });
  ...
  if (!inspection.valid || inspection.extractedFrames.length < 5) {
    const message = inspection.issueReason || '真实视频抽帧不足，无法执行身份 QA。';
    return await updateTask(task.taskId, {
      status: 'FAILED', stage: 'identity_qa',
      error: { code: 'OUTPUT_INVALID', stage: 'identity_qa', message, retryable: false, recommendedAction: '保留当前失败证据并检查生成视频/ffmpeg，不允许把未完成身份 QA 的视频标记为成功。' },
    });
  }
  if (!task.inputBucket || !task.inputObjectPath) {
    return await updateTask(task.taskId, {
      status: 'FAILED', stage: 'identity_qa',
      error: { code: 'IDENTITY_QA_UNAVAILABLE', stage: 'identity_qa', message: '缺失持久化的输入身份锚点。', retryable: false, recommendedAction: '重新创建 Identity Safe 任务；不得跳过身份 QA。' },
    });
  }

  let referenceImage: Buffer;
  let report: MvpIdentityQaReport;
  try {
    referenceImage = await exactDownload(task.inputBucket, task.inputObjectPath);
    report = await qaVideoIdentityAgainstInput({
      projectId: PROJECT_ID,
      location: REGION,
      model: task.identityQaModel || IDENTITY_QA_MODEL,
      referenceImage,
      referenceMimeType: task.inputMimeType,
  ...
  const reportPatch: Partial<MvpVideoTask> = {
    identityReport: report,
    firstAttemptIdentityReport: attempt === 1 ? report : task.firstAttemptIdentityReport || null,
  };
  task = await updateTask(task.taskId, reportPatch);
  if (report.pass && report.gateStatus === 'pass') return completeFromBuffer(task, buffer, report);

  await persistAttemptAuditArtifact(task.taskId, attempt, buffer);
  ...
  if (allowIdentityRetry && shouldRetryIdentity(report, attempt)) {
    ...
    task = await updateTask(task.taskId, {
      status: 'RETRYING', stage: 'submit', providerAttempt: 2, identityRetryCount: 1,
      retryReason: `IDENTITY_DRIFT: min=${report.minimumFaceSimilarityScore}, mean=${report.meanFaceSimilarityScore}, temporal=${report.temporalConsistencyScore}`,
      effectivePrompt: retryPrompt,
      providerStorageUri: providerStorageUri(task.taskId, 2),
      operationName: null,
      error: null,
    });
    try {
      const operationName = await submitVeoWithSafeRateRetry({
        ...
        mimeType: task.inputMimeType,
        durationSeconds: task.durationSeconds,
        taskId: task.taskId,
        attempt: 2,
      });
      return await updateTask(task.taskId, { status: 'GENERATING', stage: 'polling', operationName, error: null });
    } catch (error) {
      const classified = classifySubmitFailure(error);
      if (classified.status === 'SUBMISSION_OUTCOME_UNKNOWN') {
        return await updateTask(task.taskId, { status: classified.status, stage: 'submit', error: classified.detail });
      }
      return await updateTask(task.taskId, {
  ...
  return await updateTask(task.taskId, {
    status: 'FAILED', stage: 'identity_qa',
    error: {
      code: 'IDENTITY_DRIFT', stage: 'identity_qa',
      message: `身份 QA 未通过：gate=${report.gateStatus}, min=${report.minimumFaceSimilarityScore}, mean=${report.meanFaceSimilarityScore}, temporal=${report.temporalConsistencyScore}, visible=${report.visibleFrameRatio}.`,
      retryable: false,
      recommendedAction: attempt >= 2 ? '已完成唯一一次保守自动重试；请更换更清晰/更正面的身份锚点或进入 P0-07 首帧增强评估。' : '恢复流程不允许自动重新提交；请从正常生成入口重新创建任务。',
...
async function recoverFromProviderPrefix(task: MvpVideoTask): Promise<{ task: MvpVideoTask; found: boolean }> {
  ...
  if (candidates.length > 1) {
    const failed = await updateTask(task.taskId, { error: { code: 'OUTPUT_MISSING', stage: 'output_fetch', message: `当前 attempt-${attempt} 发现 ${candidates.length} 个视频，无法安全绑定。`, retryable: false, recommendedAction: '人工核实 GCS 产物；恢复接口绝不自动重新提交 Veo。', technicalMessage: candidates.join(', ') } });
...
<input id="identitySafe" type="hidden" value="true"/><button id="go">Identity Safe 生成</button></section>
<section><div id="headline">尚未开始</div><pre id="status">{}</pre><button id="recover" style="display:none">恢复当前 Attempt 产物（不重新提交 Veo）</button></section>
<section id="videoBox" style="display:none"><video id="video" controls></video><p><a id="download" style="color:#c4b5fd" download>下载通过身份 QA 的视频</a></p></section>
<script>
const $=id=>document.getElementById(id);let taskId=null;let timer=null;
function show(d){$('status').textContent=JSON.stringify(d,null,2);$('headline').textContent=d.status?('状态：'+d.status+(d.identityReport?' | 身份QA：'+d.identityReport.gateStatus:'')):'状态未知';$('headline').className=d.status==='COMPLETED'?'ok':(d.status==='FAILED'?'bad':(d.status==='RETRYING'||d.status==='QUALITY_CHECKING'?'warn':''));$('recover').style.display=d.status==='SUBMISSION_OUTCOME_UNKNOWN'?'inline-block':'none';if(d.videoUrl){$('videoBox').style.display='block';$('video').src=d.videoUrl;$('download').href=d.videoUrl+'?download=1';}}
...
$('go').onclick=async()=>{const image=$('image').files[0];const prompt=$('prompt').value.trim();if(!image||!prompt){alert('请选择图片并填写 Prompt');return}$('go').disabled=true;$('videoBox').style.display='none';const fd=new FormData();fd.append('image',image);fd.append('prompt',prompt);fd.append('durationSeconds',$('duration').value);fd.append('identitySafeMode','true');const key=crypto.randomUUID();try{const r=await fetch('/api/mvp/videos/start',{method:'POST',headers:{'x-idempotency-key':key},body:fd});const d=await r.json();show(d);taskId=d.taskId||null;if(taskId&&!['COMPLETED','FAILED','SUBMISSION_OUTCOME_UNKNOWN'].includes(d.status)){timer=setInterval(poll,4000);}else{$('go').disabled=false}}catch(e){show({status:'FAILED',error:{message:String(e)}});$('go').disabled=false}};
...
export async function createMvpAppV02() {
  const app = express();
  ...
  app.get('/api/mvp/health', (_req, res) => res.json({ status: 'ok', mode: 'MVP_IDENTITY_SAFE_V02', identitySafeVersion: IDENTITY_SAFE_VERSION, provider: MVP_PROVIDER, model: MVP_VIDEO_MODEL, identityQaModel: IDENTITY_QA_MODEL, projectId: PROJECT_ID, region: REGION, outputBucketConfigured: Boolean(OUTPUT_BUCKET), commitSha: process.env.K_REVISION || process.env.COMMIT_SHA || null }));

  app.get('/api/mvp/readiness', async (_req, res) => {
    const result: any = { ready: false, mode: 'MVP_IDENTITY_SAFE_V02', identitySafeVersion: IDENTITY_SAFE_VERSION, provider: MVP_PROVIDER, model: MVP_VIDEO_MODEL, identityQaModel: IDENTITY_QA_MODEL, projectId: PROJECT_ID, region: REGION, outputBucket: OUTPUT_BUCKET || null, auth: 'unknown', firestore: 'unknown', gcs: 'unknown', ffmpeg: 'configured', vertexRoute: 'unknown', commitSha: process.env.K_REVISION || process.env.COMMIT_SHA || null };
    try {
      ...
      await db.collection('_mvp_health').doc('identity-safe-v02').get(); result.firestore = 'ok';
  ...
  app.post('/api/mvp/videos/start', upload.single('image'), async (req, res) => {
    try {
      ...
      const identitySafeMode = String(req.body?.identitySafeMode ?? 'true').toLowerCase() !== 'false';
      ...
      if (identitySafeMode) {
        const safeCheck = await validateIdentitySafeInput({ imageBuffer: file.buffer, prompt });
        if (!safeCheck.pass) return res.status(400).json({ status: 'FAILED', error: inputError('IDENTITY_INPUT_UNSAFE', safeCheck.issues[0]?.message || 'Identity Safe 输入不满足要求。', JSON.stringify(safeCheck)) });
      }
      const effectivePrompt = identitySafeMode ? buildIdentitySafePrompt(prompt, false) : prompt;
      const created = await createTaskIdempotently({ idempotencyKey, prompt, effectivePrompt, identitySafeMode, durationSeconds, imageBuffer: file.buffer, mimeType: file.mimetype });
      if (created.existing) return res.status(200).json(publicTask(created.task));

      let task = created.task;
      try {
        const objectPath = await persistInputReference(task.taskId, file.buffer, file.mimetype);
        task = await updateTask(task.taskId, { inputBucket: OUTPUT_BUCKET, inputObjectPath: objectPath, status: 'SUBMITTING', stage: 'submit' });
      } catch (error: any) {
        const message = redactSecrets(error?.message || String(error));
        task = await updateTask(task.taskId, { status: 'FAILED', stage: 'artifact_persist', error: { code: 'OUTPUT_PERSIST_FAILED', stage: 'artifact_persist', message: `输入身份锚点持久化失败: ${message}`, retryable: true, recommendedAction: '检查 GCS 权限后重新创建任务；不允许在没有持久化身份锚点时提交 Identity Safe 视频。', technicalMessage: message } });
        return res.status(502).json(publicTask(task));
      }

      try {
        const operationName = await submitVeoWithSafeRateRetry({ prompt: effectivePrompt, imageBuffer: file.buffer, mimeType: file.mimetype, durationSeconds, taskId: task.taskId, attempt: 1 });
      ...
      return res.status(500).json({ status: 'FAILED', error: { code: 'INTERNAL_ERROR', stage: 'internal', message: redactSecrets(error?.message || String(error)), retryable: false, recommendedAction: '检查 Cloud Run / Firestore 日志。' } });
    }
  });
  ...
  app.get('/api/mvp/videos/:taskId/stream', async (req, res) => {
    try {
      ...
      assertIdentitySafeCompletion(task);
      ...
      if (isDownload) res.setHeader('Content-Disposition', `attachment; filename="zaojing_identity_safe_${task.taskId}.mp4"`);
      if (range) {
...
export async function startMvpServerV02() {
  const app = await createMvpAppV02();
  return app.listen(PORT, '0.0.0.0', () => console.log(`[MVP_IDENTITY_SAFE_V02] listening on :${PORT}; provider=${MVP_PROVIDER}; model=${MVP_VIDEO_MODEL}; qa=${IDENTITY_QA_MODEL}; project=${PROJECT_ID}; region=${REGION}; bucket=${OUTPUT_BUCKET || 'MISSING'}`));
}

if (process.env.NODE_ENV !== 'test' && !process.env.VITEST) void startMvpServerV02();

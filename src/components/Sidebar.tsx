...
import { Archive, Clapperboard, Film, Users, History, Settings, Settings2, Sparkles, ClipboardCheck, Activity, Image as ImageIcon, ListVideo, ListOrdered, FileJson } from 'lucide-react';

export type NavTab = 'director' | 'shot-list' | 'keyframes' | 'keyframe-assets' | 'keyframe-qa' | 'video-blueprint' | 'assets' | 'director-context' | 'monitor' | 'studio' | 'characters' | 'review' | 'history' | 'settings' | 'project-settings';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange }) => {
  const navItems = [
    { id: 'director' as NavTab, label: '导演台', icon: Clapperboard, desc: '创意、脚本与初始 Storyboard' },
    { id: 'shot-list' as NavTab, label: 'Shot List', icon: ListOrdered, desc: '前插 / 后插 / 调整 order，随时修改剧情' },
    ...
    { id: 'project-settings' as NavTab, label: '项目设置', icon: Settings2, desc: '配置项目级 Production Format Policy' },
